...
- **OpenCode Hubs / this config:** run the Bash call with a generous timeout, or background it
  (`… &`) and poll for `result.json` — the run is done when the file exists with a `status`.
- **Plain shell / other agents:** run it in the foreground for short tasks, or background it and poll
  ...
  in PowerShell, `start /b` in cmd). A pre-run usage error — bad args or an empty brief — instead exits
  with code 2 and writes no result file, so check the exit code too. A missing `opencode` binary exits
  127 but *does* write a `result.json` with status `opencode_unavailable`.
